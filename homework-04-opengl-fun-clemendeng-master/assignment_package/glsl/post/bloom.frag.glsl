...
vec4 getCol(float x, float y) {
    x /= u_Dimensions[0];
    y /= u_Dimensions[1];
    vec4 col = texture(u_RenderedTexture, vec2(clamp(fs_UV[0] + x, 0, 1), clamp(fs_UV[1] + y, 0, 1)));
    if(0.2126 * col[0] + 0.7152 * col[1] + 0.0722 * col[2] > 0.5) {
        return col;
    }
    return vec4(0);
}
...
void main()
{
    ...
    color = vec3(texture(u_RenderedTexture, fs_UV) + c);
}
