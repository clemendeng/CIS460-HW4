...
void Mesh::createCube(const char *textureFile, const char *bgTextureFile)
{
    ...
    std::vector<glm::vec4> posFront {glm::vec4(-1, -1, 1, 1),
                                     glm::vec4(1, -1, 1, 1),
                                     glm::vec4(1, 1, 1, 1),
                                     glm::vec4(-1, 1, 1, 1)};

    std::vector<glm::vec4> norFront {glm::vec4(0, 0, 1, 0),
                                     glm::vec4(0, 0, 1, 0),
                                     glm::vec4(0, 0, 1, 0),
                                     glm::vec4(0, 0, 1, 0)};

    std::vector<glm::vec4> posBack {glm::vec4(1, -1, -1, 1),
                                    glm::vec4(-1, -1, -1, 1),
                                    glm::vec4(-1, 1, -1, 1),
                                    glm::vec4(1, 1, -1, 1)};

    std::vector<glm::vec4> norBack {glm::vec4(0, 0, -1, 0),
                                    glm::vec4(0, 0, -1, 0),
                                    glm::vec4(0, 0, -1, 0),
                                    glm::vec4(0, 0, -1, 0)};

    std::vector<glm::vec4> posLeft {glm::vec4(-1, -1, -1, 1),
                                    glm::vec4(-1, -1, 1, 1),
                                    glm::vec4(-1, 1, 1, 1),
                                    glm::vec4(-1, 1, -1, 1)};

    std::vector<glm::vec4> norLeft {glm::vec4(-1, 0, 0, 0),
                                    glm::vec4(-1, 0, 0, 0),
                                    glm::vec4(-1, 0, 0, 0),
                                    glm::vec4(-1, 0, 0, 0)};

    std::vector<glm::vec4> posRight {glm::vec4(1, -1, 1, 1),
                                     glm::vec4(1, -1, -1, 1),
                                     glm::vec4(1, 1, -1, 1),
                                     glm::vec4(1, 1, 1, 1)};

    std::vector<glm::vec4> norRight {glm::vec4(1, 0, 0, 0),
                                     glm::vec4(1, 0, 0, 0),
                                     glm::vec4(1, 0, 0, 0),
                                     glm::vec4(1, 0, 0, 0)};

    std::vector<glm::vec4> posTop {glm::vec4(-1, 1, 1, 1),
                                   glm::vec4(1, 1, 1, 1),
                                   glm::vec4(1, 1, -1, 1),
                                   glm::vec4(-1, 1, -1, 1)};

    std::vector<glm::vec4> norTop {glm::vec4(0, 1, 0, 0),
                                   glm::vec4(0, 1, 0, 0),
                                   glm::vec4(0, 1, 0, 0),
                                   glm::vec4(0, 1, 0, 0)};

    std::vector<glm::vec4> posBottom {glm::vec4(-1, -1, -1, 1),
                                      glm::vec4(1, -1, -1, 1),
                                      glm::vec4(1, -1, 1, 1),
                                      glm::vec4(-1, -1, 1, 1)};

    std::vector<glm::vec4> norBottom {glm::vec4(0, -1, 0, 0),
                                      glm::vec4(0, -1, 0, 0),
                                      glm::vec4(0, -1, 0, 0),
                                      glm::vec4(0, -1, 0, 0)};

    std::vector<glm::vec2> uvsFront {glm::vec2(0, 0),
                                     glm::vec2(1, 0),
                                     glm::vec2(1, 1),
                                     glm::vec2(0, 1)};

    std::vector<glm::vec2> uvsBack = uvsFront, uvsLeft = uvsFront, uvsRight = uvsFront,
                            uvsTop = uvsFront, uvsBottom = uvsFront;

    std::vector<glm::vec4> pos = posFront;
    pos.insert(pos.end(), posBack.begin(), posBack.end());
    pos.insert(pos.end(), posLeft.begin(), posLeft.end());
    pos.insert(pos.end(), posRight.begin(), posRight.end());
    pos.insert(pos.end(), posTop.begin(), posTop.end());
    pos.insert(pos.end(), posBottom.begin(), posBottom.end());

    std::vector<glm::vec4> nor = norFront;
    nor.insert(nor.end(), norBack.begin(), norBack.end());
    nor.insert(nor.end(), norLeft.begin(), norLeft.end());
    nor.insert(nor.end(), norRight.begin(), norRight.end());
    nor.insert(nor.end(), norTop.begin(), norTop.end());
    nor.insert(nor.end(), norBottom.begin(), norBottom.end());

    std::vector<glm::vec2> uvs = uvsFront;
    uvs.insert(uvs.end(), uvsBack.begin(), uvsBack.end());
    uvs.insert(uvs.end(), uvsLeft.begin(), uvsLeft.end());
    uvs.insert(uvs.end(), uvsRight.begin(), uvsRight.end());
    uvs.insert(uvs.end(), uvsTop.begin(), uvsTop.end());
    uvs.insert(uvs.end(), uvsBottom.begin(), uvsBottom.end());
